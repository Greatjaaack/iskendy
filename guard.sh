#!/bin/sh
# Сторож iskendy.ru и аналитики: чинит что может сам, о результате пишет в чат.
#
...
# -------------------------------------------------- аналитика жива?
# Табло тянет заказы из аналитики (`/api/orders/today`): ляжет она — и заказы
# перестанут появляться на экране гостя, а сам сайт при этом будет отвечать 200.
# Поэтому сторож проверяет и её. Симптом «на табло нет заказов» до 25.09.2026
# приходилось разбирать руками, потому что аналитику не проверял никто.
#
# Перезапускаем только бэкенд аналитики: фронт и общий Caddy тут не при чём, а
# контейнер бэкенда поднимается сам и при старте досинкивает пропущенное.
ANALYTICS_URL=${ANALYTICS_URL:-https://analytics.iskendy.ru/api/health}
analytics_alive() {
  _code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -k \
          --resolve analytics.iskendy.ru:443:127.0.0.1 "$ANALYTICS_URL" 2>/dev/null)
  [ "$_code" = "200" ]
}
...
if analytics_alive; then
  ...
  if analytics_alive; then
    ...
    if analytics_alive; then
      ...
      log "аналитика: перезапуск помог"
    else
      once analytics_down "🔴 Аналитика не отвечает, перезапуск не помог. Заказы на табло не появляются."
      log "аналитика: перезапуск НЕ помог"
    fi
  fi
fi
