...
def build_text(date: str, dengi: dict | None = None) -> str:
    ...
    if not total:
        # Пусто на табло ещё не значит, что точка не работала. Когда планшет
        # кассы теряет связь или поллер молчит, заказы просто не доезжают — а
        # выручка на кассе есть. Написать в такой день «заказов не было» значит
        # соврать владельцу и спрятать поломку: цифры из аналитики показываем,
        # а пробел называем пробелом.
        if dengi and dengi.get("revenue"):
            return "\n".join([
                head, "",
                f"Выручка: {_rubli(dengi['revenue'])}",
                f"Чеков: {dengi['checks']}",
                f"Средний чек: {_rubli(dengi['avg_check'])}",
                "",
                "⚠️ На табло не попало ни одного заказа.",
                "Проверьте связь планшета кассы.",
            ])
        return f"{head}\n\nЗаказов не было."
