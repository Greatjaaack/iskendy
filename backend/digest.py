...
def build_text(date: str, dengi: dict | None = None) -> str:
    ...
    # Строка про «открытый» — только если заказы в нём вообще бывали. С 26.08
    # поллер заводит их сразу в «готовится», и для новых дней это всегда ноль:
    # печатать «Открытый: 0 мин» каждый день значит приучать пролистывать сводку.
    if st["open"]:
    ...
    if tabl:
        ...
        lines += ["", "<b>По часам приёма</b>",
                  *(["<i>Отк — ждёт готовки</i>"] if "Отк" in tabl else []),
                  "<i>Гот — готовится</i>",
                  "<i>Ждёт — ждёт гостя</i>",
